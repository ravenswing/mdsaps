...
def fes2D(fes_path, save_path, units='A', basins=None, funnel=None,
    ...
    fig.update_layout(height=1600, width=2200,
                      title_text="",
                      font=dict(color=colours.labels,
...
def fes1D(fes_path: str, cv: str, save_path: str, 
    ...
    ax.set_title(f'1D Free Energy Surface', c=colours.labels)
    fig.savefig(save_path, bbox_inches='tight',
                dpi=config.dpi, transparent=config.transparency)
    plt.close()


def cvs(colvar_path, save_path, cvs, cv_labels, units='A', title='CV Diffusion',
        xlims=None, ylims=None, mean=False, initial=False):
    ...
    for i, cv in enumerate(cvs):
        ax[i].scatter(colvar.time.multiply(0.001),
                      ...
                      c=colours.default, s=8, alpha=.4)
        if mean:
            ax[i].axhline(colvar[cv].mean()*10, ls='--', label='Mean', c=colours.highlight)
        if initial:
            ax[i].axhline(colvar[cv].iloc[0]*10, ls='dotted', label='Initial', c=colours.ax)
        if xlims:
            ax[i].set_xlim(xlims)
        if ylims:
            ax[i].set_ylim(ylims)
...
def hills(hills_path, save_path, units='A', title='Hill Heights',
...
def convergence(fes_dir: str, cv: str, times: list, rew_path: str, save_path: str,
                cv_label: str = 'CV', rew_label: str = 'Reweight') -> None:
    ...
    for i, t in enumerate(times):
        df = pd.read_table(f"{fes_dir}/", comment='#', sep='\s+', names=[cv, 'free', 'err'])

        ax.plot(df[cv].multiply(10), df.free.divide(4.184), c=colours.rainbow[i], label=f"{(t+1)*50} ns")
    
    rew = pd.read_table(rew_path, comment='#', sep='\s+', names=[cv, 'free', 'err'])
    ax.plot(rew[cv].multiply(10), rew.free.divide(4.184), c='k', ls='--', label=rew_label)
