...
def setup_minimisation(wd, pdb_filename, nm_pos_in_path, restraints=False):
    ...
    # TODO -> Copy files
    try:
        subprocess.run(["cp", f"{SCRIPT_DIR}/min*", wd + "/"], check=True)
    except subprocess.CalledProcessError as error:
        print(
            "Error code:", error.returncode, ". Output:", error.output.decode("utf-8")
        )
...
def make_restraints_list(wd, txt_in):
    ...
    for z, restr in df.iterrows():
        ...
        # variable parameters
        ir_line = (
            f"  ixpk= 0, nxpk= 0, iat={restr.i1},{restr.i2}, "
...
def main():
    ...
    for pocket in POCKETS:
        for path in glob(f"{DATA_DIR}/{pocket.lower()}/*"):
            make_reducing_restraints(
                path, "/home/rhys/SHIP/Data/SHIP Project - Restraints.csv"
            )
            transfer(path)
