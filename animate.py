...
def animate_fes(fes_path, colvar_path, out_path):
    ...
    def plot(i, xi, yi):
        if i > 0:
            fig, (ax1, ax2) = plt.subplots(2, 1, height_ratios=[3, 2],
                                           figsize=[5.3, 8],
                                           layout='constrained')
            ax1.contour(data[0], data[1], data[2], conts, colors='k',
                        linewidths=0.5, linestyles='dashed', alpha=0.6,
                        antialiased=True)
            ax1.contourf(data[0], data[1], data[2], conts, cmap='RdYlBu',
                         alpha=0.6, antialiased=True)
            ax1.plot(xi, yi, alpha=0.7, linewidth=0.8, c='k')
            ax1.scatter([xi.iloc[-1]], [yi.iloc[-1]], s=60, alpha=.9,
                        c='#ffffff', edgecolor='k', linewidths=.5, zorder=3)
            ax1.scatter([xi.iloc[0]], [yi.iloc[0]], s=80, marker='X', alpha=.9,
                        c='#e05252', edgecolor='k', linewidths=.5, zorder=3)
            ax1.set_xlim((-1, 45))
            ax1.set_ylim((-1, 50))
            ax1.set_xlabel(f'Projection / {ANG}')
            ax1.set_ylabel(f'{cv2} / {ANG}')
            ax1.text(0.95, 0.95, f'{len(xi)*scale:.1f} ns',
                     horizontalalignment='right', verticalalignment='center',
                     transform=ax1.transAxes, size=12)

            ax2.plot(np.arange(0, len(xi))*scale, xi, linewidth=0.5, c='#5252e0')
            ax2.scatter([len(xi)*scale], [xi.iloc[-1]], s=40, alpha=.9,
                        c='#ffffff', edgecolor='k', linewidths=.5, zorder=3)
